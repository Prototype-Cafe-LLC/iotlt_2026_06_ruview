...
このリポジトリだけ `git clone` し、**同階に ESP-IDF / esp-csi / RuView を取る**スクリプトを実行する（いずれも
`.gitignore` 済みで **このリポジトリにはコミットされない**）。

```bash
git clone https://github.com/<org>/iotlt_2026_06_ruview.git
cd iotlt_2026_06_ruview
chmod +x scripts/bootstrap-dev-env.sh scripts/apply-esp-csi-patches.sh
./scripts/bootstrap-dev-env.sh
# RuView 不要なら: ./scripts/bootstrap-dev-env.sh --skip-ruview
```

- ESP-IDF は **v6.0.1** を `--recursive` で取得（所要は回線次第で長め）。
- esp-csi は **パッチ前提の固定コミット**に checkout したうえで [git-apply](https://git-scm.com/docs/git-apply)
  相当の `./scripts/apply-esp-csi-patches.sh` が走る（詳細は `patches/README.md`）。
- スクリプト末尾に出るとおり、続けて `esp-idf-v6.0.1` で `./install.sh esp32s3` と `export.sh` を実行する
  （[Getting Started](https://docs.espressif.com/projects/esp-idf/en/v6.0.1/esp32/get-started/index.html)）。

### 手動で clone する場合

`bootstrap-dev-env.sh` と同じ配置になるよう、リポジトリルートに対して例えば次のようにする。clone 後は必ず
`./scripts/apply-esp-csi-patches.sh`（`ESP_CSI_DIR` でパスを変えてもよい）。

```bash
cd ${PROJECT_ROOT}
git clone -b v6.0.1 --recursive https://github.com/espressif/esp-idf.git esp-idf-v6.0.1
git clone https://github.com/espressif/esp-csi.git
git -C esp-csi checkout 8633d67152db2808f141cc1595970aa9cf406045
./scripts/apply-esp-csi-patches.sh
# RuView が必要なら: git clone --depth 1 https://github.com/ruvnet/RuView.git
...
# 【重要】clone 直後は ESP-IDF v6 ではビルドできない（./scripts/apply-esp-csi-patches.sh 適用後）:
...
（IDF v6 向けパッチは `apply-esp-csi-patches.sh` 適用後。各シェルで `export.sh` を source 済みであること）
